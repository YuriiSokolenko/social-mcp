...
Inspect the built artifacts directly:
...
Verify wheel metadata (`METADATA`, `WHEEL`, `RECORD`), expected package files (including `py.typed` when present), and `pyproject.toml` in the sdist. Check that no unintended tests, documentation, or secrets were packaged.
...
- Inspect the wheel and sdist contents: expected files are present and no unintended test or documentation files are included.
...
Source: https://github.com/Paldom/python-skills/blob/70aa0cf9926285dbef6ac39bc36990ece230cd09/skills/python-packaging/SKILL.md
